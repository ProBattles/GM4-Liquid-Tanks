...
#adding 
	#bucket
	execute as @e[tag=gm4LTwater,scores={gm4LiquidTanks=0..30000}] at @s if block ~ ~ ~ minecraft:hopper{Items:[{Slot:0b,id:"minecraft:water_bucket",Count:1b}]} run tag @s add gm4LTbucketadd
	...
	#bottle
	execute as @e[tag=gm4LTwater,scores={gm4LiquidTanks=0..30000}] at @s if block ~ ~ ~ minecraft:hopper{Items:[{Slot:0b,id:"minecraft:potion",Count:1b,tag:{Potion: "minecraft:water"}}]} run tag @s add gm4LTbottleadd
...
#removing
	#buckets
	execute as @e[tag=gm4LTwater,scores={gm4LiquidTanks=1000..30000}] at @s if block ~ ~ ~ minecraft:hopper{Items:[{Slot:4b,id:"minecraft:bucket",Count:1b}]} run tag @s add gm4LTbucketremove 
	execute as @e[tag=gm4LTwater,tag=gm4LTbucketremove] run scoreboard players remove @s gm4LiquidTanks 1000
	execute at @e[tag=gm4LTwater,tag=gm4LTbucketremove] run replaceitem block ~ ~ ~ container.4 minecraft:water_bucket 1
	...
	#bottle
	execute as @e[tag=gm4LTwater,scores={gm4LiquidTanks=250..30000}] at @s if block ~ ~ ~ minecraft:hopper{Items:[{Slot:4b,id:"minecraft:glass_bottle",Count:1b}]} run tag @s add gm4LTbottleremove 
	execute as @e[tag=gm4LTwater,tag=gm4LTbottleremove] run scoreboard players remove @s gm4LiquidTanks 250
	execute at @e[tag=gm4LTwater,tag=gm4LTbottleremove] run replaceitem block ~ ~ ~ container.4 minecraft:potion{Potion:"minecraft:water"} 1
