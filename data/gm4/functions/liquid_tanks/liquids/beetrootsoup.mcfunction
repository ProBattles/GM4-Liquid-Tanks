...
#adding 
	#bowl
	execute as @e[tag=gm4LTbeetrootsoup,scores={gm4LiquidTanks=0..30000}] at @s if block ~ ~ ~ minecraft:hopper{Items:[{Slot:0b,id:"minecraft:beetroot_soup",Count:1b}]} run tag @s add gm4LTbowladd
...
#removing
	#bowl
	execute as @e[tag=gm4LTbeetrootsoup,scores={gm4LiquidTanks=250..30000}] at @s if block ~ ~ ~ minecraft:hopper{Items:[{Slot:4b,id:"minecraft:bowl",Count:1b}]} run tag @s add gm4LTbowlremove 
