...
#adding 
execute as @e[tag=gm4LTmilk,scores={gm4LiquidTanks=0..30000}] at @s if block ~ ~ ~ minecraft:hopper{Items:[{Slot:0b,id:"minecraft:milk_bucket",Count:1b}]} run tag @s add gm4LTbucketadd
...
#removing
execute as @e[tag=gm4LTmilk,scores={gm4LiquidTanks=1000..30000}] at @s if block ~ ~ ~ minecraft:hopper{Items:[{Slot:4b,id:"minecraft:bucket",Count:1b}]} run tag @s add gm4LTbucketremove 
